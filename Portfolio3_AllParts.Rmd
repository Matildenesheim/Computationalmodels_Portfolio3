...
library(pacman)
p_load(tidyverse, rethinking, pastecs, Brobdingnag, rlang, CRAN)
...
##can also scale several variables like this
...
m1.2 <- rethinking::map(
  alist(
    ...
    mu <- a + b*ScaleNVIQ ,
    a ~ dnorm( 0 , 1 ) , #ADOS has a mean of approx 14 and ranges from 6-21 in the previous assignment
    b ~ dnorm( 0 , 1 ) , #we don't know much about the relationship, so putting a broad prior
    sigma ~ dunif( 0 , 3 ) #setting a quite generous SD since we don't know much
  ) ,
  data=d )
precis(m1.2)
